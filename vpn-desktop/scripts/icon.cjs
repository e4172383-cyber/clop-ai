...
function sample(x, y) {
  ...
  if (rect <= 0) {
    ...
    let background = mix([255, 132, 70], [177, 63, 25], direction);
    const glow = clamp(1 - Math.hypot(x - 0.27, y - 0.2) / 0.58);
    background = mix(background, [255, 166, 112], glow * 0.28);
  ...
  if (cMark(x, y, 0.006, 0.012)) composite(pixel, [84, 34, 18], 0.25);
  if (chevronMark(x, y, 0.006, 0.012)) composite(pixel, [84, 34, 18], 0.25);
  if (cMark(x, y)) composite(pixel, [247, 248, 255], 1);
  if (chevronMark(x, y)) composite(pixel, [255, 220, 190], 1);
