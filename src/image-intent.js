...
const RU_VISUAL_SUBJECT = /(?<![\p{L}\p{N}_])(?:девушк[\p{L}-]*|женщин[\p{L}-]*|парн[\p{L}-]*|мужчин[\p{L}-]*|человек[\p{L}-]*|персонаж[\p{L}-]*|портрет[\p{L}-]*|кот[\p{L}-]*|кошк[\p{L}-]*|собак[\p{L}-]*|город[\p{L}-]*|пейзаж[\p{L}-]*|сцен[\p{L}-]*)(?![\p{L}\p{N}_])/iu;
...
export function wantsGeneratedImage(input) {
  ...
  return orderedIntent(text, RU_VERB, RU_IMAGE)
    || orderedIntent(text, RU_VERB, RU_VISUAL_SUBJECT)
    || orderedIntent(text, EN_VERB, EN_IMAGE);
}
