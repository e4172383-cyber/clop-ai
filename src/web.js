  ...
  const server = http.createServer((req, res) => {
    ...
    if (url.pathname === '/releases.json' && req.method === 'GET') {
      return sendJson(res, 200, {
        desktop: {
          version: '2.0.9',
          url: `${PUBLIC_URL || 'https://clop-ai.onrender.com'}/downloads/Clop-Code-Setup-2.0.9.exe`,
      ...
      if (url.pathname === '/desk/chat' && req.method === 'POST') {
        ...
        readJsonBody(req, MAX_MESSAGE_BYTES).then(async (body) => {
          ...
          try {
            ...
            const r = await askModelImpl({ chat, model, effortKey, prompt, images, onDelta: sendDelta, fast, signal: requestAbort.signal });
            if (clientGone) {
