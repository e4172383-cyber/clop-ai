...
const DESKTOP_DOWNLOADS = new Set([
  ...
  'Clop-Code-2.4.8-linux-x64.tar.xz',
...
async function proxyReleaseAsset(req, res, name) {
  ...
  try {
    const requestHeaders = { 'user-agent': 'Clop-Download-Proxy/2.4.8' };
  ...
  const server = http.createServer((req, res) => {
    ...
    if (url.pathname === '/releases.json' && req.method === 'GET') {
      return sendJson(res, 200, {
        desktop: {
          version: '2.4.8',
          url: publicDownloadUrl('Clop-Code-Setup-2.4.8.exe'),
          windowsUrl: publicDownloadUrl('Clop-Code-Setup-2.4.8.exe'),
          linuxUrl: publicDownloadUrl('Clop-Code-2.4.8-linux-x64.tar.xz'),
        },
