...
test('/chat/api/message reflects the sixfold GPT allowance after recording usage', async () => {
  user.fast = false;
  ...
  assert.equal(response.status, 200);
  const body = await response.json();
  assert.equal(body.limits.gpt.short.percent, Math.round((Math.round(billable / 6) / limit) * 100));
