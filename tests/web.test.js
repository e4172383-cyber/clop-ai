...
test('serves the public desktop release page and resumable installers without dashboard auth', async () => {
  ...
  assert.match(html, /href="\/downloads\/Clop-Code-Setup-2\.4\.3\.exe"/);
  ...
  assert.equal(releaseData.desktop.version, '2.4.3');
  assert.match(releaseData.desktop.windowsUrl, /\/downloads\/Clop-Code-Setup-2\.4\.3\.exe$/);
  assert.match(releaseData.desktop.linuxUrl, /\/downloads\/Clop-Code-2\.4\.3-linux-x64\.tar\.xz$/);
