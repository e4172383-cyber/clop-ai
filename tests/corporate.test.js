...
test('corporate short window is per member and weekly window is shared across providers', () => {
  ...
  assert.equal(ownerState.states.find((x) => x.key === 'short').percent, 7);
  ...
  assert.equal(ownerState.states.find((x) => x.key === 'long').percent, 28);
  assert.equal(memberState.states.find((x) => x.key === 'long').percent, 28);
