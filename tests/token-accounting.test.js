...
test('legacy GPT events are corrected while preserving their model multiplier', () => {
  ...
  assert.equal(eventBillable(legacy, 'gpt'), 25);
  assert.equal(eventBillable({ ...legacy, billingVersion: BILLING_VERSION }, 'gpt'), Math.round(legacy.billable / 6));
...
test('version 2 GPT events are reduced when resumed context was charged repeatedly', () => {
  ...
  assert.equal(eventBillable(event, 'gpt'), 72);
});
